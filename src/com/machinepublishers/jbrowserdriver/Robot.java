...
class Robot {
  ...
  static {
    Map<String, KeyCode[]> textMapTmp = new HashMap<>();
...
//    textMapTmp.put(")", new KeyCode[] { KeyCode.SHIFT, KeyCode.DIGIT0 });
  ...
  private final AtomicReference<GlassRobot> robot = new AtomicReference<GlassRobot>();
      ...
      AppThread.exec(context.item().statusCode, () -> {
        ...
        robot.get().mouseMove(
            (int) Math.rint(Math.max(0, Math.min(stage.getScene().getWidth() - 1,
                viewportX + Math.floor(robot.get().getMouseX())))),
            (int) Math.rint(Math.max(0, Math.min(stage.getScene().getHeight() - 1,
                viewportY + Math.floor(robot.get().getMouseY())))));
        return null;
      });
    } finally {
      unlock();
      ...
      return AppThread.exec(context.item().statusCode, () -> {
        ...
        if (image == null && SettingsManager.isMonocle()) {
          try {
            ...
            final WritableImage pixels = robot.get().getScreenCapture(
                null,
                ...
                false);
            SwingFXUtils.fromFXImage(pixels, image);
          } catch (Throwable t) {
            attempt2 = t;
          }
        }
